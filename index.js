...
const RESOURCE = "/events" // Make sure to change this!
...
async function addParty(party) {
  try{
  const addPartyResponse = await fetch(API + RESOURCE, {
    method: 'POST',
    headers: {
...
async function removeParty(id) {
  try{
    const partyRemoval = await fetch( `${API + RESOURCE}/${id}`, {
    method: 'DELETE',
  });
    selectedParty = null;
    getParties();
